...
public class KafkaDlqPublisher implements DlqPublisher {
    ...
    public void publish(ProcessingInput input, Exception exception) {
        ...
        payload.put("attempt", input.attempt());
        payload.put("error", exception.getMessage());

        kafkaTemplate.send(properties.getTopic().getDlq(), input.accountId(), toJson(payload));
    }
    ...
    private String toJson(Map<String, Object> value) {
        try {
            ...
            throw new IllegalStateException("failed to serialize dlq payload", e);
        }
    }
}
